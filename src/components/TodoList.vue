<template>
  <div class="todo-list">
    ...
    <form class="addbox-wr" @submit.prevent="add">
      <input type="text" name="addbox" v-model.trim="addBox">
      ...
      <div v-if="incompleted.length" class="added-tasks">
        ...
        <ul>
          ...
          <li v-for="(todo, index) in todoList" :key="index" :class="{hide:todo.completed == true}">
            <p>
              <input type="checkbox" v-model="todo.completed">{{ todo.item }} <button @click="remove(todo)" :disabled="todo.completed == true">Delete</button>
              </p>
          </li>
